...
async def _send_media(context: ContextTypes.DEFAULT_TYPE, chat_id: int, filepath: str, caption: str):
    ...
    try:
        ...
        if file_size > TELEGRAM_MAX_FILE_SIZE:
            await context.bot.send_message(chat_id, f"⚠️ The file '{os.path.basename(filepath)}' is too large for Telegram ({file_size // 1024 // 1024}MB).")
            return

        with open(filepath, 'rb') as f:
            if file_ext in ['.jpg', '.jpeg', '.png', '.webp']:
                await context.bot.send_photo(chat_id=chat_id, photo=f, caption=caption, parse_mode=constants.ParseMode.HTML)
            elif file_ext in ['.mp4', '.mov', '.mkv', '.webm']:
                await context.bot.send_video(chat_id=chat_id, video=f, caption=caption, parse_mode=constants.ParseMode.HTML, supports_streaming=True)
            else:
                await context.bot.send_document(chat_id=chat_id, document=f, caption=caption, parse_mode=constants.ParseMode.HTML)
    except Exception as e:
        logger.error(f"Error sending media file {filepath}: {e}")
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Could not send file: {os.path.basename(filepath)}")

# This is the single handler for ALL URLs
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # No more special handling for Facebook
        ydl_opts = {
        ...
        try:
            ...
            if 'entries' in info and info['entries']:
                ...
                for i, entry in enumerate(entries):
                    ...
                    if os.path.exists(filepath):
                        ...
                        await _send_media(context, update.effective_chat.id, filepath, f"({i+1}/{len(entries)}) {caption}")
                    else:
            ...
            await status_message.edit_text(f"❌ Failed to download.\n\n<b>Reason:</b> <code>{error_message}</code>", parse_mode=constants.ParseMode.HTML)